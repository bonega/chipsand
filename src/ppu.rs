...
impl Fetcher {
    ...
    fn get_tile_adr(&self, tile_sel: bool) -> usize {
        let tile_offset = self.tile_index as usize * 16;
        let mut adr = tile_offset;
        // block 0 start differs depending on 8000 or 8800 adr-mode
        if !tile_sel && self.tile_index < 128 {
            adr += 0x1000
        }
